...
# 2. Build an opening book from real Lichess games at your rating
./lichess-import-parallel.sh 2025-05   # download + import (~3-6 hours)
./build-stats-fast.sh 30               # build stats table (~2-4 hours with PyPy)
./build-book.sh 1600                   # generate book (seconds)
...
No data pipeline needed on Mac — just copy a `.bin` book file from your Linux machine (or grab one from this repo's Releases).
...
| File | Purpose |
|---|---|
| `setup-maia2.sh` | One-shot Linux installer |
...
## How the book pipeline works

```
Lichess .pgn.zst → pzstd → awk filter → parallel Python → SQLite
                                                              ↓
SQLite → build-stats (staging + PyPy) → opening_stats table
                                                    ↓
opening_stats → build-book.sh → Polyglot .bin
```

The `awk` pre-filter scans PGN headers and drops ~60% of games (wrong rating or wrong time control) before Python ever sees them — it's 10x faster than Python for that work. The import pipeline runs with parallel Python workers at around 6,000–8,000 games per second on modern hardware.

The stats builder uses a **staging table approach**: instead of UPSERTing into a growing indexed table (which causes progressive slowdown as the B-tree deepens), it appends to a flat unindexed staging table during processing, then aggregates once at the end. Uses PyPy automatically if available — 3–5x faster than CPython for the zobrist-hashing inner loop.

Book weights are **proportionally scaled**: the most-popular move in any given position gets the Polyglot max weight of 65,535, and every other move is scaled down in proportion. Without this, popular first moves (e4, d4, Nf3, c4) would all hit the 16-bit cap and become equally weighted, destroying the relative frequencies.
